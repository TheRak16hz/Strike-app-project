...
async function run() {
  try {
    ...
    const deduplicateFoodsSql = fs.readFileSync(path.join(__dirname, '../db/update_v12_food_library_unique.sql'), 'utf8');
    ...
    console.log('Running finance fix (v9) migrations...');
    await pool.query(financeFixSql);
    console.log('Finance fix (v9) done.');

    console.log('Running finance goals position (v10) migrations...');
    await pool.query(goalsPositionSql);
    console.log('Finance goals position (v10) done.');
    ...
    console.log('Running food library deduplication (v12) migrations...');
    await pool.query(deduplicateFoodsSql);
    console.log('Food library deduplication (v12) done.');
